...
"[project]/incubation-frontend/app/api/applications/route.ts [app-route] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
async function GET(request) {
    try {
        ...
        let query = __TURBOPACK__imported__module__$5b$project$5d2f$incubation$2d$frontend$2f$lib$2f$supabaseServer$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["supabaseServer"].from("new_application").select("*").order("submitted_at", {
            ascending: false
        ...
        if (applicationIds.length) {
            const { data: reviewerAssignments, error: assignmentError } = await __TURBOPACK__imported__module__$5b$project$5d2f$incubation$2d$frontend$2f$lib$2f$supabaseServer$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["supabaseServer"].from("application_reviewers").select("application_id, reviewer_id").in("application_id", applicationIds);
            if (!assignmentError && reviewerAssignments?.length) {
                const reviewerIds = [
                    ...new Set(reviewerAssignments.map((r)=>r.reviewer_id))
                ];
                const { data: reviewers, error: reviewerError } = await __TURBOPACK__imported__module__$5b$project$5d2f$incubation$2d$frontend$2f$lib$2f$supabaseServer$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["supabaseServer"].from("user_profiles").select("id, full_name").in("id", reviewerIds);
                if (!reviewerError && reviewers) {
                    const reviewerLookup = Object.fromEntries(reviewers.map((r)=>[
                            r.id,
                            r
                        ]));
        ...
        let countQuery = __TURBOPACK__imported__module__$5b$project$5d2f$incubation$2d$frontend$2f$lib$2f$supabaseServer$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["supabaseServer"].from("new_application").select("*", {
            count: "exact",
            head: true
        });
        if (statusParam && statusParam !== "all") {
...
//# sourceMappingURL=%5Broot-of-the-server%5D__7e4af4ff._.js.map